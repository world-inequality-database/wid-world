...
generate data_quality =.
replace data_quality = 0 if method == "regional average"
replace data_quality = 1 if method == "imputed"
replace data_quality = 2 if method == "extrapolated via regression imputation"
replace data_quality = 3 if method == "interpolated"
replace data_quality = 4 if method == "augmentedwith2nddata"
replace data_quality = 5 if method == "original"
...
//---------------- 6. Export the pllin992f data --------------------------------
keep iso year p widcode value currency data_quality
...
keep iso sixlet method source year data_quality
order iso sixlet, first
...
bysort iso sixlet: assert data_quality_score == data_quality_score[1]
keep iso sixlet data_quality_score method source 
duplicates drop
isid iso sixlet
// -------------------- 6.5 Cleaning & prepare for export ----------------------
*generate author = "ASR&TN"

gen twolet=substr(sixlet, 2, 2)
gen threelet=substr(sixlet, 4, 3)

*  House cleaning
keep iso twolet threelet method source data_quality_score //extrapolation data_points
duplicates drop
compress

// adding empty imputation column 
gen str10 imputation = ""
gen str10 extrapolation = ""
gen str10 data_points = ""
gen data_quality =.

order iso twolet threelet method source data_quality imputation extrapolation data_points data_quality_score
