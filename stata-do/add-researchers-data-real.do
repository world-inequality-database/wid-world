...
assert !missing(data_quality) if (strpos(widcode, "ptinc") | strpos(widcode, "cainc")) & p!="p0p100"
...
keep iso year p widcode value author old data_quality currency
...
// apply pre-tax extrapolation/datapoints to post-tax series as well
preserve
	keep if inlist(sixlet, "aptinc")
	keep iso extrapolation data_points
	rename extrapolation extrap_ptinc
	rename data_points dp_ptinc
	duplicates drop 
	tempfile ptinc_metadata
	save `ptinc_metadata'
restore 
merge m:1 iso using `ptinc_metadata', nogen

replace extrapolation = extrap_ptinc if inlist(sixlet, "adiinc", "sdiinc", "tdiinc")
replace data_points = dp_ptinc if inlist(sixlet, "adiinc", "sdiinc", "tdiinc")
drop extrap_ptinc dp_ptinc
