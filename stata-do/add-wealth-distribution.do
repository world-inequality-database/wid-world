...
// this is used to identify duplicates in old database and new data added
keep iso year widcode
duplicates drop
tempfile finalunique
save `finalunique'
...
if strpos("AT BE BG CH CY CZ DK DE EE FI FR GB GR HU HR IS IE IT LV LT LU MT NL NO PL PT RO SK SI ES SE", iso) != 0 & strpos(fivelet, "hweal")
...
if missing(source) & strpos(fivelet, "hweal")
...
merge 1:1 iso fivelet using "`meta'", nogenerate update replace
...
merge m:1 iso year widcode using `finalunique' // to identify duplicates in iso-year-widcode that might be bringing different percentiles (eg specific top percentiles)
drop if _merge == 3 // drop obs that are in both old and new data 
drop _merge 
