...
foreach v in fdipx fdirx pinpx pinrx ptfpx ptfrx ptfrx_deb ptfrx_eq ptfrx_res ptfpx_deb ptfpx_eq {
	replace q_`v' = .              if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998)
	replace s_`v' = ""             if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998)
	replace   `v' = .              if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998)
	carryforward `v'               if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998), replace
	replace q_`v' = 1              if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998) & !mi(`v') & mi(q_`v')
	replace s_`v' = "carryfor" if (iso == "CW" & year == 1997) | (iso == "CW" & year == 1998) & !mi(`v') & mi(      s_`v')
}
...
gen             q_aux = q_nnfin
replace         q_aux = 0       if mi(q_nnfin)
quality               gdpro confc  aux, gen(q_nninc) 
drop *aux
generate      s_nninc = "gdpro,confc" + cond(missing(nnfin), "", "nnfin")
...
replace      s_ptfrx = "ptfrx_ratioptfrx/pinrx"  if flagnninc == 1 
replace      s_fdirx = "fdirx_ratiofdirx/pinrx"  if flagnninc == 1 
...
replace     s_ptfpx = "ptfpx_ratioptfpx/pinpx"  if flagnninc == 1 
replace     s_fdipx = "fdipx_ratiofdipx/pinpx" if flagnninc == 1 
drop flagnninc difnninc sh_*
...
replace s_pinrx = "fdirx,ptfrx" if !mi(fdirx) & !mi(ptfrx)
replace s_pinpx = "fdipx,ptfpx" if !mi(fdipx) & !mi(ptfpx)
...
foreach v in fdirx fdipx ptfrx ptfpx fdixa fdixd ptfxa ptfxd comrx compx ftaxx fsubx ptexa ptdxa ptrxa ptexd ptdxd ptrrx ptdrx pterx ptepx ptdpx {
	...
	replace  q_`v' = 1          if corecountry == 1 & missing(q_`v') & !missing(`v')
	replace  s_`v' = "carryfor" if corecountry == 1 & missing(s_`v') & !missing(`v')
}
...
foreach v in fdirx fdipx ptfrx ptfpx fdixa fdixd ptfxa ptfxd comrx compx ftaxx fsubx {
	so iso year
	by iso : carryforward `v' if year == $pastyear, replace
	replace q_`v'= 2          if year == $pastyear & missing(q_`v') & !missing(`v')
	replace s_`v'= "carryfor" if year == $pastyear & missing(q_`v') & !missing(`v')
}
...
foreach v in fdirx fdipx ptfrx ptfpx fdixa fdixd ptfxa ptfxd comrx compx fsubx ftaxx{
	replace        q_`v' = .   if missing(`v')
	replace        s_`v' =  "" if missing(`v')
}
...
replace s_pinrx = "fdirx,ptfrx" if !mi(fdirx) & !mi(ptfrx)
replace s_pinpx = "fdipx,ptfpx" if !mi(fdipx) & !mi(ptfpx)
replace s_comnx = "comrx,compx" if !mi(comrx) & !mi(compx)
replace s_taxnx = "fsubx,ftaxx" if !mi(fsubx) & !mi(ftaxx)
replace   ptfnx = ptfrx - ptfpx  
replace   fdinx = fdirx - fdipx 
replace   pinnx = fdinx + ptfnx
replace   pinrx = fdirx + ptfrx 
...
replace    s_nwgxd = "ptfxd,fdixd" if  !mi(ptfxd) & !mi(fdixd)
replace    s_nwnxa = "nwgxa,nwgxd" if  !mi(nwgxa) & !mi(nwgxd) 
...
gen       aux1= fsubx
replace   aux1= 0 if missing(fsubx)	
gen     q_aux1=q_fsubx
replace q_aux1=0 if missing(fsubx)
quality comrx pinrx aux1, gen(temp1)
replace q_finrx = temp1                                              if corecountry == 1 & !mi(comrx) & !mi(pinrx)
replace s_finrx =  "comrx,pinrx" + cond(missing(fsubx), "", "fsubx") if corecountry == 1 & !mi(comrx) & !mi(pinrx)
...
gen       aux2= ftaxx
replace   aux2= 0 if missing(ftaxx)	
gen     q_aux2=q_ftaxx
replace q_aux2=0 if missing(ftaxx)	
quality compx pinpx ftaxx, gen(temp2)
replace q_finpx = temp2                                             if corecountry == 1 & !mi(compx) & !mi(pinpx)
replace s_finpx = "compx,pinpx" + cond(missing(ftaxx), "", "ftaxx") if corecountry == 1 & !mi(compx) & !mi(pinpx)
...
gen       aux3= taxnx
replace   aux3= 0 if missing(taxnx)	
gen     q_aux3=q_taxnx
replace q_aux3=0 if missing(taxnx)	
quality flcin aux3, gen(temp3)
replace q_nnfin = temp3                                      if corecountry == 1 & !mi(flcin)
replace s_nnfin = "flcin"+ cond(missing(taxnx), "", "taxnx") if corecountry == 1 & !mi(flcin)
...
replace  s_nnfin = cond(missing(flcin), "", "flcin") + cond(missing(taxnx), "", ",taxnx") + cond(missing(taxnx) & missing(flcin),"assumed","")
generate s_nninc = "gdpro,confc" + cond(missing(nnfin), "", ",nnfin")  
generate s_ndpro = "gdpro,confc"
generate s_gninc = "gdpro"         + cond(missing(nnfin), "", "nnfin")
...
// Check GDP and NNINC
assert gdpro>0 if year>=1970
assert nninc>0 if year>=1970
