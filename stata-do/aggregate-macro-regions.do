...
replace data_quality=0  if !missing(value) & missing(data_quality)
replace s_="reginhouse" if !missing(value) & missing(s_)
...
replace source = string(firstyear) + "-" + string(lastyear) + ":  [URL][URL_LINK]https://wid.world/document/wid-national-accounts-series-updated-and-extended-coverage-1800-2023-world-inequality-lab-technical-note-2025-02/[/URL_LINK][URL_TEXT] Nievas, G., Piketty, T.(2025) `WID National Accounts Series: Updated and Extended Coverage 1800-2023'[/URL_TEXT][/URL]" if treat1=="np2025"

replace method = string(firstyear) + "-" + string(lastyear) + ": This data is computed by aggregating the estimated values of countries in this region; See  [URL][URL_LINK]https://wid.world/document/distributional-national-accounts-dina-guidelines-2025-methods-and-concepts-used-in-the-world-inequality-database/[/URL_LINK][URL_TEXT] Chancel L., FLores, I., Moshirf, R, Nievas, G. , Pikkety, T. (2025) `Distributional National Accounts Guidelines'[/URL_TEXT][/URL]" if missing(treat1)

replace source = string(firstyear) + "-" + string(lastyear) + ": In-house calculation;" if missing(source)
replace method = string(firstyear) + "-" + string(lastyear) + ": Observed data;" if missing(method)

drop metadata  categ treat*
