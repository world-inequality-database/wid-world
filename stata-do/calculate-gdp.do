...
merge 1:1 iso year using "$input_data_dir/taxhavens-data/GDP-selected_countries.dta", ///
	nogenerate update assert(using master match) keepusing(gdp*)
merge 1:1 iso year using "$work_data/nievaspiketty2025_gdp.dta", ///
	nogenerate update assert(using master match) keepusing(gdp*)
sort iso year
...
replace detail   =  iso + "_2005_GDP_share" if newobsCW
replace iso      = "CW"          if newobsCW

expand 2 if (iso == "AN") & inrange(year, 1970, 2004), generate(newobsSX)
replace detail   = iso + "_2005_GDP_share" if newobsSX
...
preserve
	...
	save "$work_data/ratioCWSX_AN.dta", replace
restore 
...
//------- 3.3 Generate and select growth rates
* gen variables to store final series
generate growth = .
generate growth_src = ""
...
replace detail   =  iso + "_GDP_growth_usd" if newobsBQ // here we may add a "cbs" note but the reality is that this data comes from UN
...
// ThenBonaire, Sint Eustatius and Saba will get the growth rate of Curacao
sort iso year
...
foreach v of varlist growth_np growth_un2 growth_wb /*growth_un1**/ ///
	...
	replace growth_src = growth_src+ "`v'" if (growth >= .) & (`v' < .)
...
replace growth_src = "growth_carryforward" if cf & year < $pastyear
...
merge 1:1 iso year using "$work_data/price-index.dta", ///
	nogenerate update keep(master match match_update match_conflict) keepusing(index currency) ///
...
// Housekeeping
keep iso year currency gdp gdp_maddison *_src  detail level_year 
...
replace growth2_src = "growth_Mad07_usd" if (growth_other >= .) & (growth_maddison < .)
generate growth     = cond(growth_other < ., -growth_other, -growth_maddison)
replace growth2_src = "growth_Mad07_usd" if missing(growth2_src) & !missing(gdp_maddison) 
...
//------- 4.4 Apply growth rates from Blanchet, Chancel & Gethin (2018) to expand East to 1980 [Find a note on this on the retropolate GDP.]
preserve
...
gsort iso -year
by iso: replace growth2_src="growth_Mad07_usd" if !mi(gr_bcg) & mi(gdp2)
...
// 5.  Expand the currency and level year 
...
replace level_src   =  detail   if year==level_year & !mi(detail)
replace growth2_src =  detail  if year!=level_year & !mi(detail)
