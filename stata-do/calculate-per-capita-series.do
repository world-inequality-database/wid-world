...
// Calculate per capita series (Macro Aggregates)
use "$work_data/calculate-wealth-income-ratio-output.dta", clear
...
// Reformat MER variables
gen region=1 if inlist(substr(iso, 1, 1), "X", "O") | inlist(iso,"QL", "QM","QE", "QF","QP","WO")
replace region=0 if inlist(iso,"XI","OM") 
replace iso=iso+"-MER" if region==1 & !strpos(iso,"-PPP")
drop region
