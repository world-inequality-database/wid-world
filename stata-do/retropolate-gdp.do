...
//------------- 1.5 Timor Leste with Indonesia
	...
	replace growth_srcTL = "ID_1990GDP_share" if missing(`var'TL) & year >= 1970
...
//------ 5. Export
preserve
	keep year iso gdp currency data_quality
...
foreach v in source method {
	preserve
		...
		keep iso `v' detail2
		if "`v'" == "source" {
			replace `v' = "The GDP was calculated using the following sources; " + ///
				substr(`v', 1, length(`v') - 1) + "."
		}
		else {
